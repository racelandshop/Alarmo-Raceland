...
export class UserEditorCard extends LitElement {
  ...
  async firstUpdated() {
    ...
    // AQUI VOU BUSCAR OS DADOS MAS AS CHAVES NAO ESTAO CA (antes do omit)
    if (this.item) {
  ...
  private saveClick(ev: Event) {
    ...
    if (!data.name?.length)
      ...
      if (this.item) {
        //SO QUANDO TROCAMOS O CODIGO e o this.data === data nao esta omitido mesmo depois do omit??????
        if ((data.old_code || '').length < 4) omit(data, 'old_code', 'code');
        console.log("DATA TROCADO", data);
      }
      ...
      console.log("DATA SEMPRE", data); //AQUI TEMOS A CHAVE DOS CODIGOS (this.data === data) mas nao esta nada omitido
